...
/**
 * API REST: Gestión de usuarios (en memoria) + JWT
 * Recurso: Usuario { dpi, name, email, password }
 * Endpoints:
 *  - POST   /users         (público)
 *  - POST   /login         (público → devuelve JWT 30s)
 *  - GET    /users         (protegido)
 *  - PUT    /users/:id     (protegido)   — :id ≡ dpi
 *  - DELETE /users/:id     (protegido)   — :id ≡ dpi
 *  - (compat) PUT/DELETE /users/:dpi (protegido, por si tu front los usa)
 */
...
// --- App base ---
const app = express();
app.use(cors());            // para Render/Netlify en clase está bien abierto
app.use(morgan('dev'));
 ...
 * POST /login
 * Body: { email, password }
 * Busca usuario por email y compara password (texto plano para la hoja de trabajo).
...
/**
 * POST /users (público)
 * Valida campos, formato y unicidad (dpi/email). No expone password en respuesta.
 */
...
/**
 * GET /users (PROTEGIDO)
 * Listado con filtros y paginación. Nunca expone password.
 */
